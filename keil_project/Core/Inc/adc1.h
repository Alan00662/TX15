...
uint32_t adc1_get_result(uint32_t channel);                          /* 获取ADC结果 */
uint32_t adc1_get_result_average(uint32_t channel, uint8_t times);   /* 均值滤波获取ADC结果 */
void adc1_dma_init(uint32_t memory_base);                            /* 初始化ADC DMA读取 */
void adc1_dma_enable(uint32_t length);                               /* 开启ADC DMA读取 */
void adc1_nch_dma_init(uint32_t memory_base);                        /* 初始化多通道ADC DMA读取 */
void adc1_nch_dma_enable(uint32_t length);                           /* 开启多通道ADC DMA读取 */
void adc1_oversample_init(uint32_t ratio, uint32_t right_shift);     /* 初始化ADC过采样 */
