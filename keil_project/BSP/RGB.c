...
void WS2812_Display_1(uint32_t Color, uint16_t num)
{
    ...
    HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t *)p, DMA_Data_Len); // 启动DMA传输
}
...
void WS2812_Number_4(uint32_t Color1, uint32_t Color2, uint32_t Color3, uint32_t Color4)
{
    ...
    for (uint8_t k = 0; k < 4; k++)
    {
        ...
        {
        case 0:
            p = (RGB_Buff_4 + Reste_Data) + (0 * Led_Data_Len);
            Color = Color1;
            break;
        case 1:
            p = (RGB_Buff_4 + Reste_Data) + (1 * Led_Data_Len);
            Color = Color2;
            break;
        case 2:
            p = (RGB_Buff_4 + Reste_Data) + (2 * Led_Data_Len);
            Color = Color3;
            break;
        case 3:
            p = (RGB_Buff_4 + Reste_Data) + (3 * Led_Data_Len);
            Color = Color4;
            break;
        default:;
            break;
    ...
    HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t *)RGB_Buff_4, DMA_Data_Len); // 启动DMA传输
}
